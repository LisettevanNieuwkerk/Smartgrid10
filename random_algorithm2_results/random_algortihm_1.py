...
class SmartGrid():
    ...
    def calculate_costs(self):
        ...
        for i in range(150):
            ...
            capacity = self.batteries[battery].capacity

            while True:
                # Pick random house
                house = random.randint(1,150)
                output_house = self.houses[house].max_output
                current_capacity = self.batteries[battery].currentCapacity
                possible_capacity = current_capacity + output_house

                # Check if max capacity not yet reached
                if possible_capacity <= capacity:
                    ...
                    self.batteries[battery].currentCapacity += output_house
                    house_to_battery = {'house': house, 'battery': battery, 'distance': distance, 'max_output_house': output_house, 'current_capacity_battery': self.batteries[battery].currentCapacity}
